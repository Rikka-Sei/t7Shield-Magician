...
class AuditTemplateSelfTest(unittest.TestCase):
    ...
    # ---- 本 spec（t7-magician）新增覆盖：模板原本没有 fixture 的规则 ----

    def test_missing_metadata_term_fails(self):
        spec = VALID_SPEC.replace("**受众:** 开发\n", "")
        with self._fixture(spec_text=spec) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
        self.assertNotEqual(completed.returncode, 0, completed.stdout + completed.stderr)

    def test_scoped_term_outside_sections_fails(self):
        manifest = _base_manifest()
        manifest["decisions"][0]["scoped_terms"] = ["示例系统"]
        with self._fixture(manifest=manifest) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
        self.assertNotEqual(completed.returncode, 0, completed.stdout + completed.stderr)

    def test_forbidden_occurrence_fails(self):
        manifest = _base_manifest()
        manifest["decisions"][0]["forbidden_occurrences"] = [
            {"pattern": "示例决策", "count": 1, "sections": ["9"],
             "reason": "禁止示例决策在决策日志出现"}
        ]
        with self._fixture(manifest=manifest) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
        self.assertNotEqual(completed.returncode, 0, completed.stdout + completed.stderr)

    def test_required_occurrence_count_fails(self):
        manifest = _base_manifest()
        manifest["decisions"][0]["required_occurrences"] = [
            {"pattern": "spec", "count": {"min": 5}, "sections": ["1"],
             "reason": "计数不足必须失败"}
        ]
        with self._fixture(manifest=manifest) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
    ...
    def test_go_anchor_match_passes(self):
        manifest = _base_manifest()
        manifest["spec_status"] = "authoritative"
        manifest["test_anchors"] = [{
            "name": "TestContractAnchor",
        ...
        files = {"src/contract_test.go": "func TestContractAnchor(t *testing.T) {}\n"}
        with self._fixture(manifest=manifest, files=files) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
        self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)

    def test_rust_fn_anchor_match_passes(self):
        manifest = _base_manifest()
        manifest["spec_status"] = "authoritative"
        manifest["test_anchors"] = [{
            "name": "TestContractAnchor",
            "source_globs": ["crates/**/*.rs"],
            "required": True,
        }]
        files = {"crates/proto/tests/frame.rs":
                 "#[test]\nfn TestContractAnchor() {\n    assert!(true);\n}\n"}
        with self._fixture(manifest=manifest, files=files) as (_, tmp, manifest_path):
            completed = _run(tmp, manifest_path)
        self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)
