...
// 对外门面：应用层（`t7-app`）按 `t7_protocol::<名字>` 取用这些项；其余构造细节按
// `t7_protocol::<module>::…` 访问（模块本身全部公开）。
pub use discovery::{
...
mod tests {
    ...
    /// 门面导出可用：应用层按 `t7_protocol::<名字>` 取用（spec §3.1 的单向依赖）。
    #[test]
