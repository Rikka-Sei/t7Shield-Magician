...
use magi_transport::cdb::{cdb_security_in, DISCOVERY_ALLOC_LEN, DISCOVERY_SP_SPECIFIC};
use magi_transport::transport::ScsiCdb;
...
///
/// 字节构造归 `magi-transport::cdb`，本函数只做一次调用，不在协议层复写 CDB 字节。
...
pub(crate) mod testkit {
    /// `../magiShield-protocol/analysis/probe-linux.raw` 的固化 fixture（0xb4 = 180 字节）。
    ///
...
mod tests {
    ...
    /// 真机 fixture（`../magiShield-protocol/analysis/probe-linux.raw`，经参考实现 selftest
