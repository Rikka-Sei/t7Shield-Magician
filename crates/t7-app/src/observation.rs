...
//!
//! 采样与窗口口径归 `t7-transport`（§6：30 s 窗口、500 ms 间隔、最多 60 次）；判据裁决归
//! `t7-protocol::evaluate_unlock`（§4.8）。本模块只做两者之间的搬运，不引入新的判据：
//! 分区表/挂载取窗口内任一采样，PID 取最后一个成功采样的现值。
//!
//! macOS 不轮询：平台能力由 [`crate::controller::PlatformCapability`] 裁决，本模块只做纯映射。

use t7_protocol::{evaluate_unlock, ReEnumerationObservation, UnlockEvidence};
use t7_transport::reenumeration::{
...
/// `t7_protocol::evaluate_unlock` 裁决；本函数不发送任何重枚举触发命令（AC-007）。
pub fn observe_and_evaluate<P>(
    probe: P,
    flags_before: u8,
    flags_after: Option<u8>,
) -> (
...
mod tests {
    ...
    use t7_protocol::unlock::{LOCKING_FLAGS_LOCKED, LOCKING_FLAGS_UNLOCKED};
    use t7_protocol::{PID_LOCKED, PID_UNLOCKED};
