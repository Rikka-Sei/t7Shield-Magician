//! t7-app：T7 Shield GUI 客户端的界面与线程编排（GTK4 + libadwaita）。
//!
//! 依赖方向（§3.1）：`t7-app` → `t7-protocol` → `t7-transport`；本 crate 不构造 CDB 与令牌流。
//!
...
//! 入口启用矩阵、单飞约束、口令清零等口径在无 display 环境下被 `cargo test -p t7-app` 覆盖
...
/// 应用 ID（非用户可见文案）。
pub const APP_ID: &str = "dev.rikki.T7Magician";
